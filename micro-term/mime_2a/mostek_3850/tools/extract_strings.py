...
Used to build ground-truth anchors for F8 ROM disassembly. Matched strings give
verified labels and back-pointers from whatever code loads them. CPU-agnostic —
identical to the parallel ampex-d175-terminal/tools/extract_strings.py.

Output format: one line per run.

    LOAD_ADDR  OFFSET  LEN  'text'
"""
import argparse
import sys
...
def main():
    ...
    ap.add_argument("--base", default="0x0000",
                    help="Load address (hex, e.g. 0x0400) to add to offsets.")
