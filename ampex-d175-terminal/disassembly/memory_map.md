...
| `0xAABB` | 1 B | ★ Primary SET-UP byte. Low nibble = active emulation (0..10). Upper nibble = 16-state baud-rate selector; `U4:sub_05CB` rotates it right 3 bits, indexes a 32-byte CTC-config table at `U4:0x029C`, and emits two `OUTI` writes to **Z80 CTC port `0x00`** (channel-control word + time-constant — descending TC sequence `C0, 60, 30, 20, 18, 10, 0C, 08, 06, 03` confirms a baud-rate prescaler). Preserved across emulation changes (`U4:0x1980`). Companion `U4:0x05D7` does the same for the lower nibble of `0xAABC` against CTC port `0x40` (printer/aux channel). **Not** CRTC: the SCN2672 is memory-mapped at `0x6000..0x6007` and is initialized separately — `U4:0x022B` sends command `0x10` to `0x6001` (Reset IR Pointer) then writes 11 IR bytes to `0x6000` from RAM `0xAA8E`, which `U4:0x0173` seeds from the ROM table at `U4:0x02BC` (`58 24 1B FD 98 4F 0B 2B 00 10 97` — IR5=0x4F → 80 columns, IR9=0x10 → 16 scanlines/char). (Note: `U4.sym` still labels `0x029C` as `crtc_config_table` and `0x05CB` as `crtc_upper_config` from an earlier pass; both are misnomers.) |
