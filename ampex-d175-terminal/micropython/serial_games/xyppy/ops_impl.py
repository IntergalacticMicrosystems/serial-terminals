...
def restore_z3(env, opinfo):
    # Patched: skip filename prompt, use env.save_path (single slot).
    loaded = quetzal.load_to_env(env, env.save_path)
    if loaded:
...
def save_z3(env, opinfo):
    # Patched: skip filename prompt, use env.save_path (single slot).
    saved = quetzal.write(env, env.save_path)
...
def save(env, opinfo):
    ...
    # Patched: skip filename prompt, use env.save_path (single slot).
    if quetzal.write(env, env.save_path):
        set_var(env, opinfo.store_var, 1)
    else:
        set_var(env, opinfo.store_var, 0)
